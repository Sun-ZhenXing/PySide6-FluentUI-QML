...
def generalTemplate(template_path, out_path, context):
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    template = Template(
        filename=template_path, preprocessor=[lambda x: x.replace("\r\n", "\n")]
    )
...
def exportIssFile():
    ...
    context = {"GIT_SEMVER": git_latest_tag}
    generalTemplate(
        "./.template/InstallerScript.iss.in",
        "./action-cli/InstallerScript.iss",
        context,
    )
...
def exportVersionPy():
    git_latest_tag = get_latest_tab("./")
    context = {"GIT_SEMVER": git_latest_tag}
    generalTemplate("./.template/version.py.in", "./example/version.py", context)
...
def exportFileVersionInfo():
    ...
    generalTemplate(
        "./.template/file_version_info.txt.in",
        "./action-cli/file_version_info.txt",
        context,
    )
